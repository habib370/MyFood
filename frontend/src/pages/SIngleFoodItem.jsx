...
export const SingleFoodItem = () => {
  ...
  useEffect(() => {
    const fetchSimilarItems = async () => {
      try {
        const res = await axios.get(`${url}/api/food/category/${itemId}`);
        if (res.data.ok) {
...
<div className="mt-16 pt-8 border-t border-gray-200">
  <div className="text-center mb-12">
    ...
    <p className="text-gray-600 text-lg text-center pb-5">
      More delicious options from the same category
    </p>
  </div>
  ...
  {similarItems.length === 0 ? (
    <div className="text-center py-12 bg-gray-50 rounded-2xl">
      ...
      <button 
        onClick={() => navigate('/')}
        className="mt-4 px-6 py-2 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-lg hover:opacity-90 transition-all font-medium"
      >
        Browse All Items
      </button>
    </div>
